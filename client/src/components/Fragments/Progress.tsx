...
/**
 * TODO This component is almost ready but it requires...
 * improvment, better use of shadows and colors
 * there is an issue with the roudness, perhabs due
 * to antialiases configuration. Requires research 
 */
...
const Progress = ({ porcentage }: { porcentage: number }) => {
  return (
    <Frame>
      <Bar>
        <Porcentage
          style={{ width: `${porcentage * 10}px` }}
        >{`${JSON.stringify(porcentage)}%`}</Porcentage>
      </Bar>
    </Frame>
  );
};
